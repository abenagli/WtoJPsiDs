...
bool Preselection::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(unsigned int it1 = 0; it1 < muons.size(); ++it1)
  {
    ...
    int isLoose1 = muon1.isLooseMuon();
    ...
    float iso1 = muon1.pfIsolationR04().sumChargedHadronPt + std::max(0.,muon1.pfIsolationR04().sumNeutralHadronEt+muon1.pfIsolationR04().sumPhotonEt-0.5*muon1.pfIsolationR04().sumPUPt);
    
    if( isLoose1 != 1 ) continue;
    if( fabs(dxy1) > 0.05 ) continue;
    // if( fabs(dxy1/dxyErr1) > 5. ) continue;
    if( fabs(dz1) > 0.1 ) continue;
    ...
    for(unsigned int it2 = it1+1; it2 < muons.size(); ++it2)
    {
      ...
      int isLoose2 = muon2.isLooseMuon();
      ...
      float iso2 = muon2.pfIsolationR04().sumChargedHadronPt + std::max(0.,muon2.pfIsolationR04().sumNeutralHadronEt+muon2.pfIsolationR04().sumPhotonEt-0.5*muon2.pfIsolationR04().sumPUPt);
      
      if( charge1*charge2 == 1 ) continue;
      if( isLoose2 != 1 ) continue;
      if( fabs(dxy2) > 0.05 ) continue;
      // if( fabs(dxy2/dxyErr2) > 5. ) continue;
      if( fabs(dz2) > 0.1 ) continue;
      ...
      if( std::max(pt1,pt2) < 12. ) continue;
      if( std::min(pt1,pt2) <  5. ) continue;
      
      if( (mu1+mu2).M()  <  2. ) continue;
      if( (mu1+mu2).M()  >  5. ) continue;
      if( (mu1+mu2).Pt() < 16. ) continue;
